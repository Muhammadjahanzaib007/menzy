import 'package:menzy/dbhelper/database.dart';
...
class DataBaseHelper {
  ...
  static Future<double?> getLast30DaysWeightAverage() async {
    ...
    Debug.printLog(
        "Last 30 Days Average of Weight ==> " + avgWeight!.average.toString());
  ...
  static Future<int?> getSumOfTotalModerateIntensity(List<String> date) async {
    final runningDao = _database!.runningDao;
    final sumofModerateIntensity =
        await runningDao.getTotalOfModerateIntensity(date);
    return sumofModerateIntensity!.moderateIntenseTime;
  }

  static Future<List<RunningData>> getHeartHealth(List<String> date) async {
    final runningDao = _database!.runningDao;
  ...
  Future<List<StepsData>> getAllStepsData() async {
   ...
   Future<void> deleteSteps() async {
    final stepsDao = _database!.stepsDao;
    final List<StepsData> result = await stepsDao.getAllStepsData();
    await stepsDao.deleteData(result);
    Debug.printLog("Delete Stepdata From  History ==> ");
    return;
  }
